...
async function init() {
    ...
    if (profileUser) {
        const name = profileUser.user_metadata?.name || profileUser.email.split('@')[0];
    ...
    function addRow(item) {
        if (allData.some(existing => existing.id === item.id)) return;

        allData = [...allData, item].sort(sortNewestFirst);
        renderVisibleTable();
    }
    ...
    function renderVisibleTable() {
        ...
        const visibleData = query
            ? allData.filter(item => [item.nama_logistik, item.nopol_kendaraan, item.kode_logistik, item.ukuran_ayam]
